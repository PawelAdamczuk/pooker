...
//    std::cout << char(27) << "[0G"; // Set cursor to first column
//    std::cout << char(27) << "[" << this->hand.size() << "A"; // Set cursor up x lines

    std::cout << "Chips left: " << this->money << std::endl;

    int bid = -2;
    while(bid < -1 || bid > this->money) {
        std::cout << "Please place your bid [-1 to fold, 0 to check]: ";
        std::cin >> bid;
    }
